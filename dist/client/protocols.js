...
export function parseAcpJsonRpc(raw) {
    let message;
    try {
    ...
    if (isRecord(message.error))
        return [{ type: "error", message: String(message.error.message ?? "Erreur agent") }];
    if (message.method === "ping")
        return [{ type: "heartbeat" }];
    if (message.method === "_x.ai/session/prompt_complete" || message.method === "x.ai/session/prompt_complete")
        return [{ type: "done" }];
...
export function parseAiSdkDataStream(raw) {
    ...
    for (const line of lines) {
        ...
        if (prefix === "0:" || prefix === "g:") {
            try {
                ...
                if (typeof text === "string" && text)
                    events.push({ type: prefix === "0:" ? "text" : "reasoning", text });
            }
        ...
        if (prefix === "3:") {
            try {
                ...
                events.push({ type: "error", message: payload });
            }
            continue;
        }
        if (prefix === "d:") {
