...
export function buildThreadBlocks(messages: AgentMessage[], streaming: boolean): ThreadBlock[] {
  ...
  while (index < visible.length) {
    ...
    if (message.role !== "user") {
      if (message.parts.length) blocks.push({ type: "message", message });
      index += 1;
      continue;
    }
    ...
    if (!live) {
      ...
      if (candidate) {
        let lastActivity = -1;
        candidate.parts.forEach((part, position) => { if (part.type === "tool" || part.type === "reasoning") lastActivity = position; });
